...
```
$ cat examples/cars.csv| ./gpt "show me japanese cars only"

honda,civic,2000,1000
```
```
$ cat examples/cars.csv| gpt "show sql statement to group by brand and sum price"
...
```
```
$ cat examples/notes.md| gpt 'from the following meeting notes, summarize action items for each meeting'
...
```
$ cat examples/notes.md| ./gpt 'from the following meeting notes, summarize each meeting by date and display the sentiment of the meeting'
